...
def parse_test_local(local):
    local.add_argument(
        ...
        default=path.join(context.src_dir, 'experiments', 'traces/60mbps.trace'),
    ...
    local.add_argument(
        ...
        '(default pantheon/test/12mbps.trace)')
    local.add_argument(
        '--prepend-mm-cmds', metavar='"mm-delay 15"',
    ...
    local.add_argument(
        '--extra-mm-link-args', metavar='"--uplink-queue=droptail --uplink-queue-args=packets=1 --downlink-queue=droptail --downlink-queue-args=packets=1"',
